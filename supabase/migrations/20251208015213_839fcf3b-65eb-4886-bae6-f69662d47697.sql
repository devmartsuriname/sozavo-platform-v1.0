-- Phase 10 Step 3: Extend Case Status Transition System with Hold & Payment Transitions
-- Adds T006-T011 transitions for payment processing and hold management

-- ============================================================
-- 1. UPDATE validate_case_transition function
-- ============================================================

CREATE OR REPLACE FUNCTION public.validate_case_transition(p_case_id uuid, p_target_status case_status, p_actor_id uuid, p_reason text DEFAULT NULL::text)
 RETURNS void
 LANGUAGE plpgsql
...
BEGIN
  ...
  CASE
    ...
    -- ============================================================
    -- NEW TRANSITIONS: T006-T011 (Phase 10 Step 3)
    -- ============================================================

    -- T006: approved → payment_pending
    WHEN v_current_status = 'approved' AND p_target_status = 'payment_pending' THEN
      IF NOT (
        'finance_officer' = ANY(v_actor_roles) OR
        'department_head' = ANY(v_actor_roles) OR
        'system_admin' = ANY(v_actor_roles)
      ) THEN
        RAISE EXCEPTION 'Cannot transition case: User does not have required role for this transition';
      END IF;
      -- No reason required, no additional business rules

    -- T007: payment_pending → payment_processed
    WHEN v_current_status = 'payment_pending' AND p_target_status = 'payment_processed' THEN
      IF NOT (
        'finance_officer' = ANY(v_actor_roles) OR
        'department_head' = ANY(v_actor_roles) OR
        'system_admin' = ANY(v_actor_roles)
    ...
    WHEN v_current_status = 'under_review' AND p_target_status = 'on_hold' THEN
      IF NOT (
        'case_reviewer' = ANY(v_actor_roles) OR
        'department_head' = ANY(v_actor_roles) OR
        'system_admin' = ANY(v_actor_roles)
      ...
      IF p_reason IS NULL OR LENGTH(TRIM(p_reason)) = 0 THEN
        RAISE EXCEPTION 'Cannot put case on hold: Reason is required';
      END IF;

    -- T009: approved → on_hold
    WHEN v_current_status = 'approved' AND p_target_status = 'on_hold' THEN
      IF NOT (
        'department_head' = ANY(v_actor_roles) OR
        'system_admin' = ANY(v_actor_roles)
      ) THEN
    ...
    WHEN v_current_status = 'payment_pending' AND p_target_status = 'on_hold' THEN
      IF NOT (
        'department_head' = ANY(v_actor_roles) OR
        'system_admin' = ANY(v_actor_roles)
      ) THEN
    ...
    WHEN v_current_status = 'on_hold' AND p_target_status = 'under_review' THEN
      IF NOT (
        'case_reviewer' = ANY(v_actor_roles) OR
        'department_head' = ANY(v_actor_roles) OR
        'system_admin' = ANY(v_actor_roles)
        ...
        RAISE EXCEPTION 'Cannot transition case: User does not have required role for this transition';
      END IF;
      -- No reason required for resuming from hold
...
-- ============================================================
-- 2. UPDATE perform_case_transition function
-- ============================================================
...
BEGIN
  ...
  -- Hold transitions: going into or coming out of on_hold
  v_is_hold_transition := (p_target_status = 'on_hold' OR v_old_status = 'on_hold');
  
  -- Payment transitions: approved → payment_pending OR payment_pending → payment_processed
